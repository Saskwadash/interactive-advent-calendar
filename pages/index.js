...
export default function Home() {
  ...
  if (currentCalendar) {
    const doors = Array.from({ length: 25 }).map((_, i) =>
      currentCalendar.doors && currentCalendar.doors[i]
        ? currentCalendar.doors[i]
        : defaultDoorContent[i]
    );

    return (
      <div className="min-h-screen p-8 app-container bg-gray-900 text-white">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-4xl font-bold">{currentCalendar.name}</h1>
        ...
        {openDoor && (
          ...
          >
            <div
              className="bg-white rounded-2xl p-6 w-full max-w-lg text-gray-900"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center gap-4 mb-4">
                <div className="bg-gray-900 rounded-full p-3">
                  {React.createElement(ICON_MAP[openDoor.icon] || Gift, {
                    className: "w-8 h-8 text-white"
                  })}
                </div>
                <div>
                  <h2 className="text-2xl font-bold">December {openDoor.index}</h2>
              ...
              <div className="mt-4">
                <p className="text-gray-700">
                  {openDoor.content || "Enjoy this surprise!"}
                </p>
              </div>
      ...
      {calendars.length === 0 ? (
        ...
        <div className="space-y-4 max-w-lg">
          {calendars.map((c) => (
            <div
              key={c.id}
              className="p-4 bg-white text-gray-900 rounded-lg shadow cursor-pointer"
              onClick={() => {
                setCurrentCalendar(c);
                window.history.pushState({}, "", `/calendar/${c.id}`);
              }}
            >
